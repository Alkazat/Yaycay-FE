...
import { Avatar, Badge, Card, CardBody, Select } from "@/components/ds";
...
/**
 * The Planning workspace. Planning *is* a conversation: the chat with Yaycay AI
 * is the stage (left), and the trip itinerary it is building sits alongside on
 * the right, updating as you go. The hands-on tools (add a day/moment yourself,
 * tailor each explorer) are tucked under the itinerary so the chat stays the
 * focus. The trip is the single source of truth - everything here saves onto it.
 */
  ...
  return (
    <div className="yc-stack" data-testid="planning-panel">
      ...
      >
        ...
        {/* ---- Sidebar: the itinerary it is building + hands-on tools ------ */}
        <aside className="yc-stack" data-testid="planning-sidebar">
                ...
                <div className="yc-stack" style={{ gap: "var(--space-2)" }}>
                  {trip.days.map((day) => (
                    <div key={day.id} style={{ display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
                      <Badge tone="aqua">{day.label}</Badge>
          ...
          {/* Hands-on: add it yourself (collapsed by default - chat is the focus). */}
          <Card variant="soft">
            <details data-testid="planning-manual">
              <summary style={{ cursor: "pointer", fontWeight: 800, padding: "var(--space-1) 0" }}>
                ✍️ Add a day or moment yourself
              </summary>
          ...
          {explorers.length > 0 ? (
            <Card variant="soft">
              <details data-testid="planning-toggles">
                <summary style={{ cursor: "pointer", fontWeight: 800, padding: "var(--space-1) 0" }}>
                  🎚️ What each explorer gets
                </summary>
                <div className="yc-stack" style={{ gap: "var(--space-3)", marginTop: "var(--space-3)" }}>
                    ...
                    return (
                      <div key={profile.id}>
                        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", marginBottom: "var(--space-2)" }}>
                          <Avatar name={profile.name} size={32} tone="aqua" />
                          <div style={{ flex: 1 }}>
                            ...
                            return (
                              <label
                                ...
                                style={{
                                  ...
                                  background: on ? "var(--aqua-50, #e6f6f8)" : "var(--surface-sunk)",
                                  border: on ? "2px solid var(--aqua-200, #b7e3e9)" : "2px solid var(--sand-200)",
                                  cursor: "pointer",
                                }}
                              >
                                ...
                                <span style={{ flex: 1, fontWeight: 700 }}>{meta.label}</span>
                                <input
                                  type="checkbox"
                                  checked={on}
                                  onChange={(e) => setOverride(profile.id, key, e.target.checked)}
                                  style={{ width: 22, height: 22, accentColor: "var(--aqua-500)" }}
                                />
                              </label>
                            );
                          })}
